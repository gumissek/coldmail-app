...
export default function ScheduledPage() {
  ...
  const statusConfig: Record<string, { labelKey: string; color: string; bg: string; border: string; Icon: typeof CheckCircle }> = {
    pending: { labelKey: 'scheduled.status_pending', color: 'var(--accent)', bg: 'rgba(108,99,255,0.1)', border: 'rgba(108,99,255,0.3)', Icon: Clock },
    sent: { labelKey: 'scheduled.status_sent', color: 'var(--success)', bg: 'rgba(34,197,94,0.1)', border: 'rgba(34,197,94,0.3)', Icon: CheckCircle },
    failed: { labelKey: 'scheduled.status_failed', color: 'var(--error)', bg: 'rgba(239,68,68,0.1)', border: 'rgba(239,68,68,0.3)', Icon: AlertCircle },
  };
  ...
  const deleteEmail = async (id: string) => {
    if (!confirm(t('scheduled.delete_confirm'))) return;
    try {
      const res = await fetch('/api/scheduled-emails', {
        method: 'DELETE',
      ...
      if (res.ok) {
        showToast(t('scheduled.deleted'));
        loadEmails();
      } else {
        showToast(t('scheduled.delete_error'), 'error');
      }
    } catch {
      showToast(t('scheduled.connection_error'), 'error');
    }
  };
  ...
  const processNow = async () => {
    ...
    try {
      ...
      if (data.sent > 0 || data.failed > 0) {
        showToast(t('scheduled.processed').replace('{sent}', data.sent).replace('{failed}', data.failed).replace('{remaining}', data.remaining));
      } else {
        showToast(t('scheduled.nothing_to_send'));
      }
      loadEmails();
    } catch {
      showToast(t('scheduled.process_error'), 'error');
    }
    setProcessing(false);
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
        <div className="card" style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 10, flex: '1 1 130px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{t('scheduled.pending')}</div>
          </div>
        </div>
        <div className="card" style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 10, flex: '1 1 130px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{t('scheduled.sent')}</div>
          </div>
        </div>
        <div className="card" style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 10, flex: '1 1 130px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{t('scheduled.total')}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        ...
        >
          <RefreshCw size={14} /> {t('scheduled.refresh')}
        </button>
        <button
        ...
        >
          ...
          {processing ? t('scheduled.processing') : t('scheduled.process_now')}
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: 40 }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 6 }}>
            {t('scheduled.empty_title')}
          </div>
          <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
            {t('scheduled.empty_desc')}
          </div>
        </div>
      ) : (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 }}>
                    <strong>{t('scheduled.to')}</strong> {email.to}
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 }}>
                    <strong>{t('scheduled.from_account')}</strong> {email.from_account}
                  </div>
                  <div style={{ display: 'flex', gap: 16, fontSize: 11, color: 'var(--text-muted)' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <CalendarClock size={11} />
                      {t('scheduled.scheduled_at')} {new Date(email.scheduled_date).toLocaleString(locale)}
                    </span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <Clock size={11} />
                      {t('scheduled.created_at')} {new Date(email.created_at).toLocaleString(locale)}
                    </span>
                  </div>
                  {email.next_send_after && email.status === 'pending' && (
                    <div style={{ marginTop: 4, fontSize: 11, color: 'var(--accent)' }}>
                      {t('scheduled.next_send')} {new Date(email.next_send_after).toLocaleString(locale)}
                    </div>
                  )}
                </div>
                ...
                {email.status === 'pending' && (
                  ...
                  >
                    <Trash2 size={13} /> {t('scheduled.delete')}
                  </button>
                )}
              </div>
            );
